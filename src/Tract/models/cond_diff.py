...
class DiffusionTransformer(nn.Module):
    ...
    def __init__(
        ...
        print('Running model with seeding: ', seed)
        ...
        self.positional_encoding = PositionalEncoding(model_dim, 
                                                      ...
                                                      max_len=num_points)
        if self.seed:
            self.seed_embed = nn.Sequential(
                nn.Linear(input_dim, model_dim), # input_dim is 3 for x,y,z
                nn.SiLU(),
    ...
    def forward(self, x: torch.Tensor, timesteps: torch.Tensor, context: torch.Tensor, seed_point=False) -> torch.Tensor:
        """
        Args:
        ...
        if self.seed:
            ...
            seed_emb = self.seed_embed(seed_point)
            x_embed = x_embed + seed_emb.unsqueeze(1)
